...
class SubjectRepository(
    ...
    private suspend fun getSubjectsFeaturesById(id: String): Result<Set<SubjectFeatures>> =
        runCatching {
    ...
    suspend fun getCaseStudiesBySubject(subjectId: String): Result<List<CaseStudy>> =
        runCatching {
            subjectsReference
                .document(subjectId)
                .collection(
                ...
                .get()
                .documents
                .map { it.data<CaseStudy>().copy(remoteId = it.reference.path) }
        }
    ...
    suspend fun getCaseStudyByPath(path: String): Result<CaseStudy> =
        runCatching {
            firebaseFirestore.document(path).get().data()
        }

    suspend fun getNursingDiagnosticByPath(path: String): Result<NursingDiagnostic> =
        runCatching {
            firebaseFirestore.document(path).get().data()
        }
}
